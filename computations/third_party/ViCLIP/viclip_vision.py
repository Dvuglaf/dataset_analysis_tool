...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device_id) if self.attn_mask is not None else None
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masking_prob=0.0):
        ...
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device_id), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
